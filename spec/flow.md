...
Program flow for the different executables.

## Mimetype detection (common)

- Read mime type from xattr. Attempt to read `user.mime_type` extended attribute. If present, use this mime type.
- If the xattr is not present, run the `mimetype` command on the file. If successful, store the result in `user.mime_type` xattr for future use. If the command fails, print an error to stderr and exit with a non-zero exit code.
